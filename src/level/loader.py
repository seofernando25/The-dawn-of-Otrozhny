    ...
    if isinstance(entity, Player):
        ...
        for connected_node in entity.nodes:
            try:
                ...
                pass
        return NodeData(position=position, connected_node_indices=connected_indices)
        ...
        if entity.patrolPoint is not None and isinstance(entity.patrolPoint, Node):
            try:
                ...
                pass
        return EnemyData(position=position, patrol_point_node_index=patrol_point_index)
    ...
    for idx, entity_data in enumerate(level_data.entities):
        if isinstance(entity_data, NodeData):
            node = EntityFactory.create(
                "node", (entity_data.position.px, entity_data.position.py)
            )
    ...
    for idx, entity_data in enumerate(level_data.entities):
        if isinstance(entity_data, NodeData):
            ...
            entity = EntityFactory.create_from_data(
                entity_dict, nodes_by_index=nodes_by_index
            )
...
def save_level(level_name: str, level: Level) -> None:
    ...
    level_data = LevelData(version=1, grid=grid, entities=entity_data_list)
